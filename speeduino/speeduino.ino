...
void loop()
{
      byte LOOP_TIMER; //buffer for interval timer flags

      mainLoopCount++;
      LOOP_TIMER = TIMER_mask; //take snapshot of current timer flags, those flags are cleared after they are used
    ...
    //-----------------------------------------------------------------------------------------------------
    readMAP();
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_1MS)) //Every millisecond
    {
      BIT_CLEAR(TIMER_mask, BIT_TIMER_1MS);
      ...
      if(ignitionSchedule1.Status == RUNNING) { if( (configPage4.useDwellLim) && (isCrankLocked != true) && (ignitionSchedule1.startTime < targetOverdwellTime) ) { ign1EndFunction(); ignitionSchedule1.Status = OFF; } }
      if(ignitionSchedule2.Status == RUNNING) { if( (configPage4.useDwellLim) && (isCrankLocked != true) && (ignitionSchedule2.startTime < targetOverdwellTime)) { ign2EndFunction(); ignitionSchedule2.Status = OFF; } }
      if(ignitionSchedule3.Status == RUNNING) { if( (configPage4.useDwellLim) && (isCrankLocked != true) && (ignitionSchedule3.startTime < targetOverdwellTime)) { ign3EndFunction(); ignitionSchedule3.Status = OFF; } }
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_15HZ)) //Every 66 milliseconds
    {
      BIT_CLEAR(TIMER_mask, BIT_TIMER_15HZ);
      readTPS(); //TPS reading to be performed every 66 milliseconds (any faster and it can upset the TPSdot sampling time)
      #if  defined(CORE_TEENSY35)       
          if (configPage9.enable_intcan == 1) // use internal can module
          {
    ...
    if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ)) //10 hertz (every 100ms)
    {
    ...
    if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_30HZ)) //30 hertz (every 33ms)
    {
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_4HZ)) //every 250ms
    {
